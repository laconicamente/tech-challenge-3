...
import { BytebankSelect } from "../ui/Select";
...
const CreateTransactionModal: React.FC<CreateTransactionModalProps> = ({
  ...
  const handleTabChange = (name: string) => setTransactionType(name);

  const handleCreateTransaction = async () => {
    console.log({ transactionType, value, category });
    if (!user) {
    ...
    try {
      ...
      Alert.prompt("Sucesso", "Transação criada com sucesso!");
      onFinished();
      onDismiss();
    } catch (error) {
                  ...
                  <ScrollView keyboardShouldPersistTaps="handled">
                    <View style={{ display: "flex", gap: 0 }}>
                      ...
                      <BytebankSelect
                        ...
                        onSelect={(v?: string) => setMethodType(v || "")}
                      ...
                      <View style={{ marginBottom: 10 }}>
                        <FileUploadButton label='Adicionar comprovante' onFinished={() => console.log('Upload finalizado')} />
                      </View>
                    </View>
                  </ScrollView>
